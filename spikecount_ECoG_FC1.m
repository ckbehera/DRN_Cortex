function spikecount_ECoG_FC1(X_IFR, labels_IFR, Fs_IFR, X_ECoG, labels_ECoG, Fs_ECoG, Nepochs, flowpass, fhighcut, file_Type)
...
den=squeeze(den);
COH = squeeze(mean(COH,2))./den;     
COH = abs(COH).^2;
%COH = reshape(COH, Nfreq, []);
...
fc = squeeze(max(Mask));
row_is_zero = all(fc==0,2);
fc=unique( fc(~row_is_zero, :), 'rows');
[rId, cId, val] = find(~row_is_zero );
